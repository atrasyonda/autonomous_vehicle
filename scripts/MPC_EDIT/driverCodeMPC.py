...
A=np.matrix([[1, 0.046, 0],[-0.046, 1, 1.84123],[0, 0, 1]])
B=np.matrix([[-0.1, 0],[0, 0], [0, -0.1]])
C=np.matrix([[1, 1, 1]])

r=3; m=2 # number of inputs and outputs
n= 3 # state dimension

# check the eigenvalues of the system
...
inputTest=np.ones((2,timeSampleTest))
x0test=np.zeros(shape=(3,1))
...
# # compute the system's step response
# inputTest=10*np.ones((1,timeSampleTest)) # input is a step, setpoint 10, dimension (1,200)
# x0test=np.zeros(shape=(4,1)) # initial state, value = 0, dimension (4,1)
...
# W3 matrix --> INPUT WEIGHTING MATRIX
W3=np.matmul(W1.T,np.matmul(W2,W1))

# W4 matrix --> PREDICTION WEIGHTING MATRIX
